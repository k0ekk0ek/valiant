...
/* GLib defines it's own tolower and toupper macros that consist of verifying
   wheter a given ASCII character is either lower or upper case followed by
   calculating and returning it's lower or upper case equivalent. instead I
   define two vectors where lower case is mapped to upper case and vice versa.
   for characters that are not considered to be alphabetic the vector will
   contain the exact same value. */

static const uint16_t ascii_lc_table_data[256] = {
...
const uint16_t * const ascii_lc_table = ascii_lc_table_data;

static const uint16_t ascii_uc_table_data[256] = {
  0x000, 0x001, 0x002, 0x003, 0x004, 0x005, 0x006, 0x007,
  0x008, 0x009, 0x00a, 0x00b, 0x00c, 0x00d, 0x00e, 0x00f,
  0x010, 0x011, 0x012, 0x013, 0x014, 0x015, 0x016, 0x017,
  0x018, 0x019, 0x01a, 0x01b, 0x01c, 0x01d, 0x01e, 0x01f,
...
const uint16_t * const ascii_uc_table = ascii_uc_table_data;

int
ascii_strcasecmp (const char *s1,
                  const char *s2)
{
  char *p1, *p2;
  int c1, c2;
  ...
  return (ascii_tolower (*p1) - ascii_tolower (*p2));
}

int
ascii_strncasecmp (const char *s1,
                   const char *s2,
                   size_t n)
{
  char *p1, *p2;
  ...
  if (n)
    return (ascii_tolower (*p1) - ascii_tolower (*s2));
  else
    return (0);
}
