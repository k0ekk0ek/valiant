...
/* The interface is inspired by libConfuse, which was replaced because it
   didn't have all the options valiant required. */
...
typedef int(*vt_config_def_validate_func_t)(vt_config_t *, vt_config_t *, int *);
...
struct _vt_config_def {
  ...
  vt_config_def_validate_func_t validate_func;
};

#define vt_config_def_set_validate_func(def,func) \
  ((def)->validate_func = func)

vt_config_t *vt_config_create (const char *, int, int *);
vt_config_t *vt_config_opt_create (const char *, int, vt_value_type_t, int *);
vt_config_t *vt_config_sec_create (const char *, int, const char *, int *);
...
#define VT_CONFIG_BOOL(name, def, flags) \
  { (name), VT_CONFIG_TYPE_OPT, (flags), \
    {.opt = {NULL, NULL, VT_VALUE_TYPE_BOOL, \
      {VT_VALUE_TYPE_BOOL, {.bln = (def) ? true : false }}}}, NULL}
#define VT_CONFIG_BOOL_LIST(name, def, flags) \
  { (name), VT_CONFIG_TYPE_OPT, ((flags) | VT_CONFIG_FLAG_LIST), \
    {.opt = {NULL, NULL, VT_VALUE_TYPE_BOOL, \
      {VT_VALUE_TYPE_BOOL, {.bln = (def) ? true : false }}}}, NULL}
#define VT_CONFIG_INT(name, def, flags) \
  { (name), VT_CONFIG_TYPE_OPT, (flags), \
    {.opt = {NULL, NULL, VT_VALUE_TYPE_INT, \
      {VT_VALUE_TYPE_INT, {.lng = (def) }}}}, NULL}
...
#define VT_CONFIG_FLOAT(name, def, flags) \
  { (name), VT_CONFIG_TYPE_OPT, (flags), \
    {.opt = {NULL, NULL, VT_VALUE_TYPE_FLOAT, \
      {VT_VALUE_TYPE_FLOAT, {.dbl = (def) }}}}, NULL}
...
#define VT_CONFIG_STR(name, def, flags) \
  { (name), VT_CONFIG_TYPE_OPT, (flags), \
    {.opt = {"\"", "\'", VT_VALUE_TYPE_STR, \
      {VT_VALUE_TYPE_STR, {.str = (def) }}}}, NULL}
...
#define VT_CONFIG_END() \
  { NULL, VT_CONFIG_TYPE_NONE, 0, \
    {.opt = {NULL, NULL, VT_VALUE_TYPE_NONE, \
      {VT_VALUE_TYPE_NONE, {.lng = 0 }}}}, NULL}

char *vt_config_getname (vt_config_t *);

int vt_config_getnvals (vt_config_t *);
...
int vt_config_getfloatn (vt_config_t *, double, int, int, int *);
int vt_config_setnfloat (vt_config_t *, double, int, int, int *);
...
vt_config_t *vt_config_sec_getnopt (vt_config_t *, const char *, int, int, int *);
